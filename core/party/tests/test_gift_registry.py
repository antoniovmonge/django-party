...
from core.party.models import Gift
...
def test_gift_registry_page_lists_gifts_for_party_by_id(
    ...
    url = reverse("party:page_gift_registry", args=[party.uuid])
...
def test_gift_detail_partial_returns_gift_detail_including_party(
    ...
    url = reverse("party:partial_gift_detail", args=[gift.uuid])
    response = authenticated_client(create_user).get(url)
...
def test_partial_gift_update_returns_gift_update_form(
    ...
    url = reverse("party:partial_gift_update", args=[gift.uuid])
    response = authenticated_client(create_user).get(url)
...
def test_partial_gift_delete_removes_gift(
    ...
    url = reverse("party:partial_gift_delete", args=[gift.uuid])
...
def test_get_partial_new_gift_returns_create_gift_form_with_party(
    ...
    url = reverse("party:partial_new_gift", args=[party.uuid])
    response = authenticated_client(create_user).get(url)
